...
export class CustomerService {
   ...
   */
  static async getMonthlyStatements(userId: string) {
    const cards = await prisma.card.findMany({
      where: { userId },
      include: {
        transactions: {
    ...
    const statementsMap: Record<string, { month: string; year: number; totalSpent: number; transactionsCount: number; cardLast4: string }> = {};
